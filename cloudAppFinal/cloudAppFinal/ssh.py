...
# remove and update and insert requiry very specific input (e.g., remove needs ObjectId), since we only have limited time to do so 
# these function were not fully completed, but we have already built a frameword of it.
...
# for analyst to get sharding info
...
# for analyst to get sharding info
def get_chunk_status(dbname):
    config = client.config
    chunkStatus = list(config.chunks.find({"ns": dbname}))
    return chunkStatus
